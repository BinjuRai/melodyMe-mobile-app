...
import 'package:batch34_b/features/lesson/domain/entity/lesson_entity.dart';
import 'package:batch34_b/features/wishlist/data/data_source/wishlist_datasourse.dart';
import 'package:batch34_b/features/wishlist/data/model/wish_lesson_api_model.dart'; // This now contains WishLessonModel
import 'package:batch34_b/features/wishlist/domain/repository/wishlist_repository.dart';
import 'package:dartz/dartz.dart';
import 'package:dio/dio.dart';
...
class WishlistRepositoryImpl implements WishlistRepository {
  ...
  // Rename method return type and usage to WishLessonModel
  WishLessonModel _entityToModel(LessonEntity entity) {
    return WishLessonModel(
      id: entity.id,
      name: entity.name,
      description: entity.description,
      authorName: entity.authorName,
  ...
  Future<Either<Failure, void>> addLessonToWishlist(
    ...
    try {
      final wishLessonModel = _entityToModel(lesson);
      await remoteDataSource.addLessonToWishlist(userId, wishLessonModel);
      return const Right(null);
    } on DioException catch (e) {
      return Left(
        ServerFailure(message: e.message ?? 'Failed to add lesson to wishlist'),
